...
if __name__ == '__main__':
    scanner = NmapScanner(name='hola', ports=range(1, 3000), targets='127.0.0.1', arguments='-A')
    ...
    for host in scanner.scanned_hosts():
        print('IP:' + host)
        print('State: ' + scanner.state(host))
        print('Reason: ' + scanner.reason(host))
        # For each host name from the host
        ...
        for proto in scanner.all_protocols(host):
            ...
            for port in scanner.scanned_ports(host, proto):
                ...
                if service is not None:
                    print(str(service))
                for name, output in scanner.port_scripts(host, proto, port):
                    print("\tScript: " + name)
                    print("\tOutput: " + output)

        for os in scanner.most_accurate_os(host):
            print('OS Match with most accuracy: {}'.format(os))
        fingerprint = scanner.os_fingerprint(host)
        if fingerprint is not None:
            print('OS Fingerprint: ' + fingerprint)
        for name, output in scanner.host_scripts(host):
            print("Host script: " + name)
            print("Output: " + output)

        for trace in scanner.trace_info(host):
            print(str(trace))
